...
class Dataset():
    ...
    def load_data(self, train_file, test_file=None, val_file=None):
        ...
        self.vocab = build_vocab_from_iterator(self.yield_tokens(train_iter, tokenizer), specials=['<unk>', '<pad>'], min_freq=5)
        self.vocab.set_default_index(self.vocab["<unk>"])
        pretrained_vectors = GloVe()
        self.word_embeddings = pretrained_vectors.get_vecs_by_tokens(self.vocab.get_itos())
